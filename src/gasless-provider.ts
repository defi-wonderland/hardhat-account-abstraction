...
export class GaslessProvider extends ProviderWrapper {
  ...
  private async getChainId(): Promise<number> {
    const rawChainId = await this._wrappedProvider.request({method: "eth_chainId", params: []}) as string;
  ...
  private async isContractDeployed(address: string): Promise<boolean> {
    const code = await this._wrappedProvider.request({method: 'eth_getCode', params: [address, 'latest']});
    return code !== '0x';
  }
  ...
  private async getProxyNextNonce(address: string): Promise<number> {
    ...
    const rawNonce = await this._wrappedProvider.request({method: "eth_call", params: [populatedNonceFetch, 'latest']}) as string;
    return parseInt(rawNonce);
  }

  private async encodeMessage(signer: ethers.Wallet, to: string, data: string, value: number, chainId: number, nonce: number): Promise<{ r: string, s: string, v: number }> {
    const encodedMessage = ethers.utils.defaultAbiCoder.encode(['address', 'bytes', 'uint256', 'uint256', 'uint256'], [to, data, value, chainId, nonce]);
    log('Encoding message', {
      to, data, value, chainId, nonce
    });
    const digest = ethers.utils.keccak256(encodedMessage);
    const rawSignature = await ethers.utils.joinSignature(signer._signingKey().signDigest(toEthSignedMessageHash(digest)));
